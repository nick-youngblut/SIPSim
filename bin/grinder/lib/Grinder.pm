...
=item -fp <file_path> | -file_path <file_path>

Path to genome files. Default: file_path.default
...
sub Grinder {
  ...
  diversity_report( $factory->{num_libraries}, 
		    $factory->{shared_perc},
  ...
  # output table header
  print join("\t", qw/library rank taxon_name scaffold_id rel_abund_perc/), "\n";
  ...
  while ( my $c_struct = $factory->next_lib ) {
    ...
    $factory->write_community_structure($c_struct, $out_ranks_file, $cur_lib);
    next;
...
sub diversity_report {
  ...
  my $format = '%.1f';
  print STDERR "Overall diversity = $overall_diversity genomes\n";
  if ($num_libraries > 1) {
    my $nof_shared  = $perc_shared / 100 * $overall_diversity;
    $perc_shared = sprintf($format, $perc_shared);
    print STDERR "Percent shared   = $perc_shared % ($nof_shared genomes)\n";
    my $nof_permuted  = $perc_permuted / 100 * $overall_diversity;
    $perc_permuted = sprintf($format, $perc_permuted);
    print STDERR "Percent permuted = $perc_permuted % ($nof_permuted top genomes)\n";
  }
  return 1;
}
...
sub write_community_structure {
  my ($self, $c_struct, $filename, $cur_lib) = @_;
  ...
  for my $species_id ( sort { $species_abs{$b} <=> $species_abs{$a} } keys %species_abs ) {
    ...
    my ($taxonName,$scafID) = split(/_:-:_/, $species_id, 2);
    print join("\t", $cur_lib, $rank, $taxonName, $scafID, $species_ab), "\n";
  }
...
sub argparse {
  ...
  # rel2abs for file path
  $ARGV{file_path} = File::Spec->rel2abs($ARGV{file_path});
...
sub database_create {
  ...
  my ($self, $ref_file, $unidirectional, $forward_reverse_primers,
  ...
  while(<IN>){
    ...
    next if /^\s*$/;
    my @l = split /\t/;
    die "ERROR: input should have >=2 columns. Instead: '$_'\n"
      if scalar @l < 2;

    # adding file path and saving in hash
    my @parts = File::Spec->splitpath($l[1]);
    $refs{$l[0]} = File::Spec->join($self->{file_path}, $parts[2])
  }
  close IN or die $!;
  ...
  # loading all genome fasta files
  ...
  while( my ($taxon_name,$fasta_file) = each %refs){
    ...
    while ( my $ref_seq = <$in> ) {  # each contig/scaffold in genome fasta
      # Skip empty sequences
      next if not $ref_seq->seq;
      # changing ref seq ID
      $ref_seq->id( join('_:-:_', $taxon_name, $ref_seq->id) );
	...
	$amp_bc = join('_:-:_', $taxon_name, $amp_bc);
	$ref_seq_id = join('_:-:_', $taxon_name, $ref_seq_id);
