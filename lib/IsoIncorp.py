...
# TODO: clean up
class IsoIncorp_OLD(object):
    """Class for setting intra-population isotope incorporation distribution parameters"""
    
    def __init__(self, comm, config, phylo=None):
        """
        Args:
        config -- configobj instance
        comm -- dataframe of comunity table
        """
        self.config = config
        self.comm = comm

        # setting the interPopDist functions
        self._set_interPopDistFuncs()
    ...
    def _set_interPopDistFuncs(self):
        ...
        for (libID,val1) in self.config.iteritems():
            for (intraPopDist,val2) in self.iter_configSections(val1):
                for (intraPopDistParam,val3) in self.iter_configSections(val2):
                    # setting standard distributions
                    for (interPopDist,val4) in self.iter_configSections(val3):
    ...
    def get_libSection(self, libID):
        ...
        libID -- library ID
        """
        return self.config[libID]

        
    def get_configLibs(self, trunc=False):
        """Get the library IDs from the config"""
        if trunc==True:
            return [re.sub('\D+', '', x) for x in self.config.keys()]
        else:
    ...
    def taxonInLib(self, taxon_name, libID):
        ...
        dfSub = self.comm.loc[self.comm['taxon_name'] == taxon_name]
        libs = [str(x) for x in dfSub['library'].tolist()]
        return libID in libs
        
                
    def get_unique_taxon_names(self):
        """Get the unique taxon names listed in the community file."""
        return self.comm['taxon_name'].unique() 

    def get_unique_libraries(self):
        """Get the unique libraries listed in the community file."""
        return self.comm['library'].unique() 


        
        
class ConfigLibDistOLD(object):
    """Setting distribution params for a library based on config-file defined params"""

    def __init__(self, libID, distID, x):
        """A distribution set in a config file for 1 library
        Args:
        libID -- library ID
        distID -- intra-population distribution library
        x -- dict of params to set the distributions for
        """
        self.libID = libID
        self.intraPopDistID = distID

        # setting intra-pop distribution if provided
        try:
            self.intraPopDist = x['distribution']            
        except KeyError:
            w = 'WARNING: no distribution provided for library{} -> distribution{}"; setting distribution=normal\n'
            sys.stderr.write(w.format(self.get_libID()))
            self.intraPopDist = 'normal'
        
        # setting weights if provided
        try:
            self.intraPopDistWeight = x['weight']            
        except KeyError:
            w = 'WARNING: no weight provided for library{} -> distribution{}"; setting weight=1\n'
            sys.stderr.write(w.format(self.get_libID(), self.get_intraPopDistID()))
            self.intraPopDistWeight = 1

            
        # loading intra-pop params and weights
        self.intraPopParams = defaultdict(dict)
        for (intraPopParam,v) in x.items():
            if intraPopParam in ['weight', 'distribution']:
                continue
            assert 'distribution' in v, '"distribution" must be set for each parameter in the config file'
            for (interPopParam,v) in v.items():
                interPopParam = interPopParam.lower()
                self.intraPopParams[intraPopParam][interPopParam] = v

                                                        
        # setting inter-population distribution functions
        psblDists = {'normal' : np.random.normal,
                     'uniform' : np.random.uniform}

        self.distFuncs = dict()
        for intraPopParam in self.intraPopParams.keys():
            distName = self.get_interPopDist(intraPopParam)
            distOpts = self.get_interPopDistParams(intraPopParam)
            try:
                distFunc = psblDists[distName]
                self.distFuncs[intraPopParam] = functools.partial(distFunc, **distOpts)
            except KeyError:
                raise KeyError('"{}" is not a supported distribution'.format(distName))
                
                
    def get_intraPopDist(self):
        return self.intraPopDist

        
    def get_distFunction(self, param):        
        """Get the distribution function for a param of interest.
        Args:
        param -- intra-pop parameter for with the distribution will be set
        Return:
        function
        """
        try:
            return self.distFuncs[param]
        except KeyError:
            raise KeyError('param "{}" do not have a set distribution function'.format(param))            

            
    def call_distFunction(self, param, *args, **kwargs):        
        """Call distribution function for a param of interest.
        Args:
        param -- intra-pop parameter for with the distribution will be set
        ...
        """
        try:
            return self.distFuncs[param](*args, **kwargs)
        except KeyError:
            raise KeyError('param "{}" do not have a set distribution function'.format(param))            
            

    def iter_intraPopDistParams(self):
        for p in self.intraPopParams.keys():
            if p == 'distribution':
                continue
            yield p

    def get_interPopDistParams(self, intraPopParam):
        """Get the specified parameter value for the inter-population distribution
        that describes how the intra-population distribution param varies among populations.
        Args:
        param -- intra-pop parameter for with the distribution will be set
        """
        assert intraPopParam in self.intraPopParams, '"{}" not set for {}'.format(intraPopParam, self.get_libID())        
        assert 'distribution' in self.intraPopParams[intraPopParam],\
            '"distribution"not set for {}->{}'.format(self.get_libID(), intraPopParam)

        return {k:v for (k,v) in self.intraPopParams[intraPopParam].iteritems() if 'distribution' not in k}


    def get_intraPopDist(self):
        """Get distribution name for intra-population distribution
        """
        try:
            return self.intraPopDist
        except AttributeError:
            w = 'WARNING: no distribution provided for library{}; setting distribution=normal\n'
            sys.stderr.write(w.format(self.get_libID))
            return 'normal'

            
    def get_intraPopDistWeight(self):
        """Get weight param for intra-population distribution
        """
        try:
            return self.intraPopDistWeight
        except AttributeError:
            w = 'WARNING: no weight provided for library{}->distribution{}"; setting weight=1\n'
            sys.stderr.write(w.format(self.get_libID(), self.get_intraPopDistID()))
            return 1
    ...
    def get_intraPopDistID(self):
        return self.intraPopDistID
        
    def get_interPopDist(self, intraPopParam, lower=True):
        """
        Args:
        param -- intra-pop parameter for with the distribution will be set
        lower -- return value in lower case?
        """
        assert intraPopParam in self.intraPopParams, '"{}" not set for {}'.format(intraPopParam, self.get_libID())        
        assert 'distribution' in self.intraPopParams[intraPopParam],\
            '"distribution"not set for {}->{}'.format(self.get_libID(), intraPopParam)
        ...
        if lower == True:
            return self.intraPopParams[intraPopParam]['distribution'].lower()
        else:
            return self.intraPopParams[intraPopParam]['distribution']
