...
class MFEprimer(object):
    """MFEprimer api class"""

    def __init__(self, infile, database, k_value=None):
        self.infile = infile
    ...
    def run(self):
        ...
        amp_list, oligos = self.process_primer(session_dir)

        #if options.tab:
        self.tab_out(amp_list, oligos, options, start_time, session_dir)
        #else:
        #    format_output_primer(amp_list, oligos, options, start_time, session_dir)
	...
	def print_head(self, out, options):
    ...
    def primer_analysis(self, product, oligos, session_dir, fcdict, db):
    ...
    def cal_PPC(self, f_match, p_len, r_match, m_len):
        '''Cal PPC parameter'''
    ...
    def format_output_primer(self, amp_list, oligos, options, start_time, session_dir):
    ...
    def print_foot(self, out, options, start_time):
    ...
    def draw_graphical_alignment_primer(self, amp, oligos, options, mid_seq):
    ...
    def get_mid_seq(self, mid_seq_id_list, options, session_dir, db):
    	...
    	print msg
            self.print2stderr(msg)
        
        if err:
            print err
            self.print2stderr(err)
    ...
    def get_position(self, mer_id, db):
    ...
    def check_infile(self):
        ...
        err_or_degenerate = SeqCheck.fasta_format_check(self.infile)
        ...
        if degenerate == 'no':
            oligos = FastaFormatParser.parse(self.infile)
        else:
            oligos = DegenerateSeqConvetor.convert(FastaFormatParser.parse(self.infile))
    
        self.infile.close()
    
        return oligos
    
    def primer_process(self, session_dir, fcdict, db, oligos):
        '''Primer Process'''
        ...
        for oligo in oligos:
            ...
            mer = primer_seq[-self.k_value:]
            mer_id = chilli.DNA2int(mer)
    
            # p for plus strand, m for minus strand
            p_pos_list, m_pos_list = get_position( mer_id, db)
        ...
        for i in xrange(len(oligos)):
            ...
            for k in xrange(len(oligos)):
                ...
                for j in p_list.iterkeys():
                    ...
                    for p in p_pos:
                        left = self.get_pos_range(p, m_pos)
    ...
    def get_pos_range(self, value, list_com):
    ...
    def extract_by_twoBitToFa(self, id_pos_range_list, options, session_dir, db):
        '''Get the amp sequence using twoBitToFa from Blat suite'''
        seq_list = []
    ...
    def Thermodynamics_alignment(self, fp, ts, primer_type):
        '''Alignment'''
        fp = fp.upper()
        ts = ts.upper()
        aseq = [':',] # Alignment seq
    ...
    def get_align_seq(self, seq_list, options, product):
        '''Alignment seq'''
        filter_product = []
        for i in xrange(len(product)):
            amp = product[i]
    ...
    def tab_out(self, amp_list, oligos, options, start_time, session_dir):
        '''Format output in primer task'''
    ...
    def process_primer(self, session_dir):
        '''Primer task'''
        if not isinstance(self.database, list):
            self.database = [self.database]
    
        if not isinstance(self.infile, file):
            self.infile = open(self.infile)
    
        oligos = self.check_infile()

        amp = []
        for db in self.database:
            fcdict_cache = db + '.uni'
            fcdict = chilli.get_cache(fcdict_cache)
            product, binding_range = self.primer_process(session_dir, fcdict, db, oligos)
